...
class AuthService {
  ...
  async getAWSCredentials(): Promise<CredentialsResponse> {
    ...
    if (now - this.lastRefresh < 1000) {
      throw new Error('Too many credential refresh attempts. Please try again in a moment.');
    }
    ...
    if (!CREDENTIALS_API_URL) {
      ...
      throw new Error('Credentials API URL not configured. Please set REACT_APP_CREDENTIALS_API_URL in your environment.');
    }
    ...
    try {
      ...
      try {
        ...
        console.warn('OPTIONS preflight failed, continuing with main request:', e);
      }
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to get AWS credentials: ${response.status} ${response.statusText} - ${errorText}`);
      }
      ...
      if (!data.credentials || !data.credentials.accessKeyId || !data.credentials.secretAccessKey) {
        console.error("Invalid credentials format received:", JSON.stringify(data, null, 2));
        throw new Error("Received invalid credentials format from the server");
      }
      
      console.log("Successfully received credentials with expiration:", data.credentials.expiration);
