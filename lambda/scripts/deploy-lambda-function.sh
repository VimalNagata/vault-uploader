...
if [ -z "$1" ]; then
  echo "Usage: ./deploy-lambda-function.sh <function-name> [--optimized]"
  exit 1
fi
...
if aws lambda get-function --function-name "$FUNCTION_NAME" --region "$REGION" >/dev/null 2>&1; then
  ...
  # Update function code with optimized or standard package
  if [ "$OPTIMIZED" == "true" ]; then
    aws lambda update-function-code \
      --function-name "$FUNCTION_NAME" \
      --zip-file "fileb://../dist/$FUNCTION_NAME-optimized.zip" \
      --region "$REGION"
  else
    aws lambda update-function-code \
      --function-name "$FUNCTION_NAME" \
